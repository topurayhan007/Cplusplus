...
//Partition Size is determined here
...
void printHistogram(int* fullarr, int flen, int*& smallArr, int** hist, int* numOfElem, int bin, int partitionSize, int min, int max) {
	...
	int slen = 0;
	//if I want Sir's answer then do below
	//float start = 0; float end = partitionSize;
	...
	for (int i = 0; i < bin; ++i) {
		...
		*/
		//For printing bars in a histogram instead of numbers in a range
		for (int j = 0; j < numOfElem[i]; ++j) {
			cout << (char)219;
		}
		cout << endl;
...
int main() {
	string filename = "D:/CodeBlocks Code/input.txt";
	int bin = 3;
